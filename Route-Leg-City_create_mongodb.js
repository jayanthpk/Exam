...
class Route {
  ...
  async inputLegs() {
    const uri =
      'mongodb+srv://jayanth:jayanth123@cluster0.tjshzg1.mongodb.net/test?retryWrites=true&w=majority';
    ...
    try {
      ...
      for (let i = 0; i < this.numLegs; i++) {
        ...
        await client.db('route').collection('legs').insertOne(legData);
      }
    } catch (error) {
  ...
  async calculateTotalCost() {
    ...
    try {
      ...
      var legsCollection = client.db('route').collection('legs');
